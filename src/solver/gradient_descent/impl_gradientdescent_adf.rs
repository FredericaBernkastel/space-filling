...
impl<T> GradientDescent<T, f64> where T: BorrowMut<ADF> {
  ...
  pub fn insert_sdf(&mut self, sdf: impl Fn(Point2D<f64, WorldSpace>) -> f64 + Send + Sync + 'static) -> bool {
    let domain = Rect::new(
      Point2D::splat(0.0),
      Size2D::splat(1.0),
    );
  ...
  pub fn insert_sdf_domain(&mut self, domain: Rect<f64, WorldSpace>, sdf: impl Fn(Point2D<f64, WorldSpace>) -> f64 + Send + Sync + 'static) -> bool {
    let ret = self.dist_field.borrow_mut().insert_sdf_domain(domain, Arc::new(sdf));
